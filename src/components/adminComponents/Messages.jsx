...
export default function Messages() {
  ...
  return (
    <div className=" w-full grid grid-cols-4 gap-4 tablet:grid-cols-8 pc:grid-cols-12 grid-auto-row tablet:gap-6 pc:gap-8 px-2 tablet:px-4 pc:mx-auto pc:px-8 max-w-[1636px]">
      <h1 className="col-span-2 text-deepsea font-header font-bold">
        Latest Messages
      </h1>
      {data.data.map((message) => (
        <div
          className=" px-4 font-text  col-span-4 tablet:col-span-8 pc:col-span-12 flex flex-col border-b border-tigerlily pb-5 max-w-96 "
          key={message.id}
        >
